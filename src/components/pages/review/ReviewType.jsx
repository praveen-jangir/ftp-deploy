...
import './rec.css'
import { writeReview } from './reviewData'
import { useLocation } from 'react-router-dom';
import { Helmet } from 'react-helmet'
import { Col, Container, Row } from 'react-bootstrap'


export default function ReviewType() {
    const location = useLocation();
    const queryParams = new URLSearchParams(location.search);
    const paramValue = queryParams.get('id');
    const source_url = queryParams.get('source_url');
    const [service, setSearvice] = useState('');
    ...
    formDataFile.append('service',service);
    formDataFile.append('rate',rate);
    formDataFile.append('comment',comment);
    formDataFile.append('name',name);
    formDataFile.append('email',email);
    formDataFile.append('service_id',paramValue);
    formDataFile.append('user_id',userId);
    try {
      const response = await fetch('https://www.assignmentexperthelp.co.uk/admin/reviews-post', {
        method: 'POST',
        body: formDataFile,
      });
  ...
  const changeHandle = (name, value) => {
    ...
    else if(name=='rate')setRate(rate);
    else if(name=='service')setSearvice(value);
    else if(name=='email')setEmail(value);
  }
    return (
    <>
        <Helmet>
            <link rel="canonical" href='https://www.assignmentexperthelp.co.uk/write-review'></link>
            <link rel="alternate" href='https://www.assignmentexperthelp.co.uk/write-review' hreflang="en-gb"></link>
        </Helmet>
        <Breadcrumb />
       <div className="drop_review recently_added ">
            <Container>
                <form className="form-roll" onSubmit={handleSubmit}>
                    <input type="hidden" name="_token" />
                    <Row>
                        <Col lg={7} md={12}>
                            <div className="review_form">
                                <div className="form-trol">
                                    <h4 className="form-ttl">Submit Your Feedback</h4>
                                    <Diamond />
                                    <Row>
                                        {
                                            writeReview.map((item, index) => {
                                                const { col, labelFor, labelText, lengthUnit, type, name, uniqe } = item;
                                                return(
                                                    <div key={index} className={`form-group ${col}`}>
                                                        <label htmlFor={labelFor}>{labelText}</label>
                                                        {
                                                            type === 'textarea' ? (
                                                                <textarea maxlength={lengthUnit} required="" type={type} name={name} id={uniqe} onChange={(e)=>changeHandle(name,e.target.value)}
                                                                className="form-control" ></textarea> 
                                                            ) : (
                                                                <input maxlength={lengthUnit} required="" type={type} name={name} id={uniqe} onChange={(e)=>changeHandle(name,e.target.value)}
                                    ...
                                    <div className="btn-flex text-right">
                                        <button className="btn btn-subb" type="submit">Review</button>
                                    </div>
                                </div>
                            </div>
                        </Col>
                            ...
                            <div className="heading text-center">
                                <div className="feedback">
                                    <div className="rating">
                                        <input type="radio" name="rate" id="rating-5" value="5" required="" onChange={(e)=>setRate(e.target.value)}/>
                                        <label for="rating-5"></label>
                                        <input type="radio" name="rate" id="rating-4" value="4" onChange={(e)=>setRate(e.target.value)}/>
                                        <label for="rating-4"></label>
                                        <input type="radio" name="rate" id="rating-3" value="3" onChange={(e)=>setRate(e.target.value)}/>
                                        <label for="rating-3"></label>
                                        <input type="radio" name="rate" id="rating-2" value="2" onChange={(e)=>setRate(e.target.value)}/>
                                        <label for="rating-2"></label>
                                        <input type="radio" name="rate" id="rating-1" value="1" onChange={(e)=>setRate(e.target.value)}/>
                                ...
                                <h2 className="ttl">Could you give me a performance review for my work</h2>
                                <p>Hi, I am your expert and would like to know your feedback on this assignment.</p>
                            </div>
                        </Col>
                    </Row>
                </form>
